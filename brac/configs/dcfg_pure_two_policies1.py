...
def get_data_config(env_name, policy_root_dir=None):
  ...
  ckpt_file_ours = os.path.join(
      policy_root_dir,
      env_name,
      'sac/1/agent_partial_target',
      )

  ckpt_file_authors = os.path.join(
      os.path.dirname(os.path.realpath(__file__)),
      '../../trained_policies',
      env_name,
      'agent_partial_target')

  p1_ours_pure = ['load', ckpt_file_ours, ['none',], model_params]
  p1_authors_pure = ['load', ckpt_file_authors, ['none',], model_params]

  data_config = [
      ['p1_ours_pure', p1_ours_pure, 1],
      ['p1_ours_pure', p1_authors_pure, 1]
  ]
  return data_config
